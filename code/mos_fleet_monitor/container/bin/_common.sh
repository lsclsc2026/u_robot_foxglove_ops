...
NORMAL_COMPONENTS=(vision navigation image_relay status_logger bridge)
TELEOP_COMPONENTS=(vision navigation image_relay status_logger teleop bridge)
ALL_COMPONENTS=(vision navigation image_relay status_logger teleop bridge)
STOP_COMPONENTS=(teleop bridge status_logger image_relay navigation vision)

case "${MOS_MONITOR_MODE:-normal}" in
  normal) COMPONENTS=("${NORMAL_COMPONENTS[@]}") ;;
  teleop) COMPONENTS=("${TELEOP_COMPONENTS[@]}") ;;
  *)
    echo "未知监控模式：${MOS_MONITOR_MODE}（允许 normal 或 teleop）" >&2
...
declare -A OWNED_MARKERS=(
  ...
  [teleop]="${COMPONENT_SUPERVISOR} teleop"
...
declare -A LEGACY_OWNED_MARKERS=(
  ...
  [teleop]="${MOS_MONITOR_ROOT}/assets/foxglove_teleop_guard.py"
...
declare -A CONFLICT_PATTERNS=(
  ...
  [teleop]="mos_ops_control/ops_control|mos_ops_control ops_control|foxglove_teleop_guard.py"
