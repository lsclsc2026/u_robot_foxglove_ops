...
required=(
  ...
  /root/work_space/mos_fleet_monitor/assets/foxglove_teleop_guard.py
)
...
if (( check_only )); then
  ...
  if docker exec "${CONTAINER_NAME}" zsh -lc '
    source /opt/ros/humble/setup.zsh
    source /tmp/mos_all_test_ws/install/setup.zsh
    python3 -c "import cv2, cv_bridge, diagnostic_msgs, geometry_msgs, mos_hal, rclpy, std_msgs"
  ' >/dev/null 2>&1; then
    echo "[通过] Python 模块 cv2、cv_bridge、diagnostic_msgs、geometry_msgs、mos_hal、rclpy、std_msgs"
  else
    echo "[缺失] 遥控或监控所需 Python 模块中至少一项不可用" >&2
    check_failed=1
  fi
...
install -m 0755 "${PACKAGE_ROOT}/host/bin/mos-teleop-start" \
  "${HOST_TARGET}/bin/mos-teleop-start"

docker exec "${CONTAINER_NAME}" mkdir -p -- "${CONTAINER_TARGET}"
docker cp "${PACKAGE_ROOT}/container/." \
  "${CONTAINER_NAME}:${CONTAINER_TARGET}/"

# Install the additive operations node into the existing workspace. This does
# not modify navigation, SLAM, arm, or vendor camera packages.
docker exec "${CONTAINER_NAME}" mkdir -p \
  /tmp/mos_all_test_ws/src/mos_ops_control
docker cp "${PACKAGE_ROOT}/ops_extension/mos_ops_control/." \
  "${CONTAINER_NAME}:/tmp/mos_all_test_ws/src/mos_ops_control/"
docker exec "${CONTAINER_NAME}" zsh -lc '
  set -e
  source /opt/ros/humble/setup.zsh
  cd /tmp/mos_all_test_ws
  colcon build --packages-select mos_ops_control --symlink-install
'
docker exec "${CONTAINER_NAME}" chmod 0755 \
