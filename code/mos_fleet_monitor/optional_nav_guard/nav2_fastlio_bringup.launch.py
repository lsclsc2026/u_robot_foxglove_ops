"""Single-instance guard for the original Lumos Nav2/FAST-LIO launch."""
...
LOCK_DIR = Path("/run/lock/mos_navigation.instance")
LOCK_PID_PATH = LOCK_DIR / "pid"
PID_PATH = Path("/run/mos_navigation.pid")
STATUS_PATH = Path("/run/mos_navigation.status")
ORIGINAL_LAUNCH = Path(__file__).with_name(
...
def _process_is_live_navigation(pid: int) -> bool:
    ...
    return (
        state != "Z"
        and b"ros2 launch lumos_nav" in command
        and b"nav2_fastlio_bringup.launch.py" in command
    )
...
def _acquire_single_instance_lock() -> None:
    ...
    for _attempt in range(3):
        try:
            ...
            if existing_pid is not None and _process_is_live_navigation(
                existing_pid
            ):
                message = "禁止打开多个导航 launch：导航系统已在运行。"
                print(f"\n[禁止] {message}\n", flush=True)
    ...
    if not _owns_lock:
        raise RuntimeError(f"无法取得导航单实例目录锁：{LOCK_DIR}")
...
def generate_launch_description():
    ...
    if not ORIGINAL_LAUNCH.is_file():
        raise RuntimeError(f"找不到原始导航启动文件：{ORIGINAL_LAUNCH}")
