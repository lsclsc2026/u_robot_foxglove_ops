...
if [[ -s "${pid_file}" ]]; then
    ...
    if [[ "${vision_pid}" =~ ^[0-9]+$ ]] && [[ -d "/proc/${vision_pid}" ]]; then
        ...
        state="$(awk '{print $3}' "/proc/${vision_pid}/stat" 2>/dev/null || true)"
        if [[ "${state}" != "Z" ]] && [[ "${cmdline}" == *"ros2 launch mos_3d_object vision.launch.py"* ]]; then
            if [[ -r "${status_file}" ]]; then
                echo "正在运行：$(<"${status_file}")"
            else
                echo "正在运行：pid=${vision_pid}"
            fi
            exit 0
        fi
    fi
...
echo "未运行：vision.launch.py 当前可以启动。"
exit 3
