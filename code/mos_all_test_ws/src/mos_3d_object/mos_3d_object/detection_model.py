...
class ObjectDetector:
    ...
    """
    def __init__(self, model_size='small', weights=None, conf_thres=0.25, iou_thres=0.45, classes=None, device=None):
        """
        Initialize the object detector
        
        Args:
            model_size (str): Model size ('nano', 'small', 'medium', 'large', 'extra')
            weights (str): Optional path to a custom Ultralytics YOLO .pt model
        ...
        model_name = weights if weights else model_map.get(model_size.lower(), model_map['small'])
        
        # Load model
        try:
            self.model = YOLO(model_name)
            if weights:
                print(f"Loaded custom YOLO model from {weights} on {self.device}")
            else:
    ...
    def detect(self, image, track=True):
        ...
        try:
            ...
            print(
                f"TIMING yolo_predict stage=predict event=start "
                f"track={bool(track)} device={self.device}",
                flush=True,
            )
            if track:
            ...
            predict_t1 = time.perf_counter()
            print(
                f"TIMING yolo_predict stage=predict event=end "
                f"track={bool(track)} device={self.device} "
                f"duration_ms={(predict_t1 - predict_t0) * 1000.0:.1f}",
                flush=True,
            )
            ...
            if self.device == 'mps' and "not currently implemented for the MPS device" in str(e):
                ...
                print(
                    f"TIMING yolo_predict stage=predict event=start "
                    f"track={bool(track)} device=cpu fallback=true",
                    flush=True,
                )
                if track:
